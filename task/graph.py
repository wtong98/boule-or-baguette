...
def samp_child(node, dist):
    low = node * 2**dist
    high = node * 2**dist + 2**dist
    return np.random.randint(low, high)

key = jax.random.PRNGKey(9)
jax.random.randint(key, (3,), jnp.array([1, 10, 100]), jnp.array([9, 90, 900]))
# TODO: use jax to sample children <-- STOPPED HERE
...
class BinaryTreeTiTask:
    def __init__(self, depth, n_dims=None, samp_dist=1, batch_size=128) -> None:
        assert batch_size % 2 == 0, 'batch size must be even'
        ...
        # self.cnx = fast_binary_pow(self.adj, self.n_nodes)
    ...
    def __next__(self):
        root_depths = np.random.choice(self.depth - self.samp_dist, size=self.batch_size)
        range_low = 2**root_depths
        range_high = 2**(root_depths + 1) - 1
