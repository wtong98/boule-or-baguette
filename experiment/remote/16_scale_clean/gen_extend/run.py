...
for lr, n_hop_prop, n_arm, depth, n_hidden, n_layer in itertools.product(lrs, n_hop_props, n_arms, n_depths, n_widths, all_n_layer):
    ...
    def make_chain(cot=True, ttr=False, invert_some=False):
        task_args = {
            'depth': depth,
            'samp_dist': (1, n_hop),
            'n_arms': n_arm,
        ...
        return StarfishTask(cot=cot, trace_to_start=ttr, invert_some=invert_some, **task_args)
    ...
    all_cases.extend([
        ...
        Case(f'AR (with inv)',
                TransformerConfig(n_heads=1,
                                ...
                                **model_args),
                train_args=make_train_args('ce_mask'),
                train_task=make_chain(cot=True, ttr=False, invert_some=True),
                info={'n_hop_prop': n_hop_prop, 'n_hop': n_hop}
        ), 
        ...
        Case(f'DP (with inv)',
                TransformerConfig(n_heads=1,
                                ...
                                **model_args),
                train_args=make_train_args('bce'),
                train_task=make_chain(cot=False, invert_some=True),
                info={'n_hop_prop': n_hop_prop, 'n_hop': n_hop}
        ), 
    ])
